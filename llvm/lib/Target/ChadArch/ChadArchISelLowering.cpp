...
#include "ChadArch.h"
#include "ChadArchMachineFunctionInfo.h"
...
static const MCPhysReg ArgGPRs[] = {ChadArch::R9, ChadArch::R10, ChadArch::R11,
                                    ChadArch::R12};

void ChadArchTargetLowering::ReplaceNodeResults(
...
ChadArchTargetLowering::ChadArchTargetLowering(const TargetMachine &TM,
  ...
  setStackPointerRegisterToSaveRestore(ChadArch::R1);
...
#include "ChadArchGenCallingConv.inc"
...
SDValue
ChadArchTargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
  ...
  CCInfo.AnalyzeCallOperands(Outs, CC_ChadArch);
  ...
  for (unsigned i = 0, j = 0, e = ArgLocs.size(); i != e; ++i) {
    ...
    if (VA.isRegLoc()) {
      ...
      if (!StackPtr.getNode())
        StackPtr = DAG.getCopyFromReg(Chain, DL, ChadArch::R1, PtrVT);
      SDValue Address =
  ...
  Chain = DAG.getNode(ChadArchISD::CALL, DL, NodeTys, Ops);
  ...
  RetCCInfo.AnalyzeCallResult(Ins, RetCC_ChadArch);
...
static SDValue convertValVTToLocVT(SelectionDAG &DAG, SDValue Val,
                                   ...
                                   const ChadArchSubtarget &Subtarget) {
...
static SDValue unpackFromRegLoc(SelectionDAG &DAG, SDValue Chain,
                                const CCValAssign &VA, const SDLoc &DL,
                                const ChadArchTargetLowering &TLI) {
  MachineFunction &MF = DAG.getMachineFunction();
...
/// ChadArch formal arguments implementation
SDValue ChadArchTargetLowering::LowerFormalArguments(
  ...
  SmallVector<CCValAssign, 16> ArgLocs;
  CCState CCInfo(CallConv, IsVarArg, MF, ArgLocs, *DAG.getContext());
  CCInfo.AnalyzeFormalArguments(Ins, CC_ChadArch);
  ...
  if (IsVarArg) {
    ...
    const TargetRegisterClass *RC = &ChadArch::GPRRegClass;
    MachineFrameInfo &MFI = MF.getFrameInfo();
    MachineRegisterInfo &RegInfo = MF.getRegInfo();
    ChadArchFunctionInfo *UFI = MF.getInfo<ChadArchFunctionInfo>();
...
bool ChadArchTargetLowering::CanLowerReturn(
  ...
  if (!CCInfo.CheckReturn(Outs, RetCC_ChadArch))
    return false;
...
SDValue
ChadArchTargetLowering::LowerReturn(SDValue Chain, CallingConv::ID CallConv,
                                    bool IsVarArg,
  ...
  const ChadArchSubtarget &STI = MF.getSubtarget<ChadArchSubtarget>();
  ...
  CCInfo.AnalyzeReturn(Outs, RetCC_ChadArch);
  ...
  return DAG.getNode(ChadArchISD::RET, DL, MVT::Other, RetOps);
}
...
SDValue ChadArchTargetLowering::PerformDAGCombine(SDNode *N,
...
bool ChadArchTargetLowering::isLegalAddressingMode(const DataLayout &DL,
